import { Readable } from 'node:stream'
import { pipeline } from 'node:stream/promises'
import ytdl from '@distube/ytdl-core'

const PIPED_FALLBACKS = [
  'https://api.piped.private.coffee',
  'https://pipedapi.kavin.rocks',
  'https://pipedapi.nosebs.ru',
  'https://pipedapi.darkness.services',
  'https://pipedapi.orangenet.cc',
]

const INVIDIOUS_INSTANCES = [
  'https://invidious.privacydev.net',
  'https://invidious.fdn.fr',
  'https://vid.puffyan.us',
  'https://invidious.protokolla.fi',
  'https://yt.artemislena.eu',
]

const STREAM_CACHE_TTL_MS = 45 * 60 * 1000
const streamCache = new Map()

let pipedInstancesCache = null
let pipedInstancesFetchedAt = 0

function cacheKey(videoId, iosSafe) {
  return iosSafe ? `${videoId}:ios` : videoId
}

function isIosSafeMime(mime = '') {
  const m = mime.toLowerCase()
  if (!m) return false
  if (m.includes('webm') || m.includes('opus')) return false
  if (m.includes('mpegurl') || m.includes('application/x-mpegurl')) return false
  return m.includes('mp4') || m.includes('m4a')
}

function isDirectPlayUrl(url = '') {
  return /proxy\.piped\.|pipedproxy/i.test(url)
}

async function getPipedInstances() {
  const merged = new Set(PIPED_FALLBACKS)

  if (pipedInstancesCache && Date.now() - pipedInstancesFetchedAt < 300_000) {
    pipedInstancesCache.forEach((url) => merged.add(url))
    return [...merged]
  }

  try {
    const response = await fetch('https://piped-instances.kavin.rocks/', {
      signal: AbortSignal.timeout(5000),
    })
    if (response.ok) {
      const list = await response.json()
      list
        .filter((item) => item.api_url && item.uptime_24h > 50)
        .forEach((item) => merged.add(item.api_url.replace(/\/$/, '')))
    }
  } catch {
    /* use fallbacks only */
  }

  pipedInstancesCache = [...merged]
  pipedInstancesFetchedAt = Date.now()
  return pipedInstancesCache
}

function pickStreamUrl(data, iosSafe = false) {
  if (data.livestream) return null

  const audioCandidates = (data.audioStreams || []).filter((stream) => stream.url)
  const mp4Audio = audioCandidates
    .filter((stream) => {
      const mime = stream.mimeType || ''
      const fmt = (stream.format || '').toUpperCase()
      return isIosSafeMime(mime) || fmt === 'M4A'
    })
    .sort((a, b) => (a.bitrate || 0) - (b.bitrate || 0))[0]
  if (mp4Audio?.url) {
    return { url: mp4Audio.url, contentType: mp4Audio.mimeType || 'audio/mp4' }
  }

  if (!iosSafe) {
    const anyAudio = audioCandidates.sort((a, b) => (a.bitrate || 0) - (b.bitrate || 0))[0]
    if (anyAudio?.url) {
      return { url: anyAudio.url, contentType: anyAudio.mimeType || 'audio/mp4' }
    }
  }

  const progressive = (data.videoStreams || []).filter((stream) => {
    if (!stream.url || stream.videoOnly) return false
    const quality = (stream.quality || '').toUpperCase()
    const fmt = (stream.format || '').toUpperCase()
    const mime = stream.mimeType || ''
    const url = stream.url
    if (quality.includes('LBRY') || quality.includes('HLS')) return false
    if (fmt === 'HLS' || mime.includes('mpegurl')) return false
    if (url.includes('odycdn.com')) return false
    if (iosSafe && !isIosSafeMime(mime)) return false
    return fmt === 'MPEG_4' || fmt === 'MP4' || mime.includes('video/mp4')
  })

  for (const quality of ['144p', '240p', '360p', '480p']) {
    const match = progressive.find((stream) => stream.quality === quality)
    if (match?.url) {
      return { url: match.url, contentType: match.mimeType || 'video/mp4' }
    }
  }

  const combined = progressive.sort((a, b) => (a.bitrate || 0) - (b.bitrate || 0))[0]
  if (combined?.url) {
    return { url: combined.url, contentType: combined.mimeType || 'video/mp4' }
  }

  return null
}

async function pipedStreamUrl(videoId, iosSafe = false) {
  const instances = await getPipedInstances()

  const results = await Promise.allSettled(
    instances.map(async (base) => {
      const response = await fetch(`${base}/streams/${videoId}`, {
        signal: AbortSignal.timeout(12_000),
        headers: { 'User-Agent': 'ANTSN/1.0' },
      })
      if (!response.ok) return null

      const data = await response.json()
      return pickStreamUrl(data, iosSafe)
    }),
  )

  for (const result of results) {
    if (result.status === 'fulfilled' && result.value) return result.value
  }

  return null
}

async function invidiousStreamUrl(videoId, iosSafe = false) {
  const results = await Promise.allSettled(
    INVIDIOUS_INSTANCES.map(async (base) => {
      const response = await fetch(
        `${base}/api/v1/videos/${videoId}?fields=adaptiveFormats,liveNow`,
        { signal: AbortSignal.timeout(10_000) },
      )
      if (!response.ok) return null

      const data = await response.json()
      if (data.liveNow) return null

      const candidates = (data.adaptiveFormats || []).filter(
        (format) => format.type?.includes('audio') && format.url,
      )

      const sorted = candidates.sort((a, b) => {
        if (iosSafe) {
          const aOk = isIosSafeMime(a.type) ? 0 : 1
          const bOk = isIosSafeMime(b.type) ? 0 : 1
          if (aOk !== bOk) return aOk - bOk
        } else {
          const aMp4 = a.type?.includes('mp4') ? 0 : 1
          const bMp4 = b.type?.includes('mp4') ? 0 : 1
          if (aMp4 !== bMp4) return aMp4 - bMp4
        }
        return (a.bitrate || 0) - (b.bitrate || 0)
      })

      const audio = iosSafe
        ? sorted.find((format) => isIosSafeMime(format.type))
        : sorted[0]

      if (audio?.url) {
        return { url: audio.url, contentType: audio.type || 'audio/mp4' }
      }
      return null
    }),
  )

  for (const result of results) {
    if (result.status === 'fulfilled' && result.value) return result.value
  }

  return null
}

export async function resolveUpstreamStream(videoId, iosSafe = false) {
  const key = cacheKey(videoId, iosSafe)
  const cached = streamCache.get(key)
  if (cached && cached.expiresAt > Date.now()) {
    return cached
  }

  if (iosSafe) {
    const invidious = await invidiousStreamUrl(videoId, true)
    if (invidious) {
      const entry = { ...invidious, expiresAt: Date.now() + STREAM_CACHE_TTL_MS }
      streamCache.set(key, entry)
      return entry
    }

    const piped = await pipedStreamUrl(videoId, true)
    if (piped) {
      const entry = { ...piped, expiresAt: Date.now() + STREAM_CACHE_TTL_MS }
      streamCache.set(key, entry)
      return entry
    }

    return null
  }

  const piped = await pipedStreamUrl(videoId, false)
  if (piped) {
    const entry = { ...piped, expiresAt: Date.now() + STREAM_CACHE_TTL_MS }
    streamCache.set(key, entry)
    return entry
  }

  const invidious = await invidiousStreamUrl(videoId, false)
  if (invidious) {
    const entry = { ...invidious, expiresAt: Date.now() + STREAM_CACHE_TTL_MS }
    streamCache.set(key, entry)
    return entry
  }

  return null
}

async function pipeRemoteUrl(upstream, res, req) {
  const headers = {
    'User-Agent':
      'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
  }
  if (req?.headers?.range) {
    headers.Range = req.headers.range
  }

  const response = await fetch(upstream.url, { headers })

  if (!response.ok && response.status !== 206) {
    throw new Error(`Upstream ${response.status}`)
  }

  const upstreamType = response.headers.get('content-type') || upstream.contentType || 'audio/mp4'
  if (upstreamType.includes('json')) {
    throw new Error('Upstream returned error response')
  }

  res.statusCode = response.status
  res.setHeader('Content-Type', upstreamType)
  res.setHeader('Cache-Control', 'public, max-age=3600')
  res.setHeader('Accept-Ranges', 'bytes')
  res.setHeader('Access-Control-Allow-Origin', '*')
  res.setHeader('Access-Control-Expose-Headers', 'Content-Length, Content-Range, Accept-Ranges')

  const contentLength = response.headers.get('content-length')
  const contentRange = response.headers.get('content-range')
  if (contentLength) res.setHeader('Content-Length', contentLength)
  if (contentRange) res.setHeader('Content-Range', contentRange)

  if (!response.body) {
    throw new Error('Upstream returned empty body')
  }

  await pipeline(Readable.fromWeb(response.body), res)
}

async function pipeFromYtdl(videoId, res) {
  const info = await ytdl.getInfo(String(videoId))

  if (info.videoDetails.isLiveContent || info.videoDetails.isLive) {
    throw new Error('Live streams cannot be layered — try a regular video')
  }

  const format = ytdl.chooseFormat(info.formats, { filter: 'audioonly' })
  if (!format?.url) throw new Error('No audio format found')

  res.statusCode = 200
  res.setHeader('Content-Type', format.mimeType || 'audio/mp4')
  res.setHeader('Cache-Control', 'no-store')
  res.setHeader('Accept-Ranges', 'bytes')
  res.setHeader('Access-Control-Allow-Origin', '*')

  await new Promise((resolve, reject) => {
    const stream = ytdl.downloadFromInfo(info, { format })
    stream.on('error', reject)
    res.on('close', () => stream.destroy())
    stream.on('end', resolve)
    stream.pipe(res)
  })
}

export function isIosSafeRequest(req) {
  if (!req) return false
  if (req.query?.mobile === '1') return true
  if (req.url) {
    try {
      const url = new URL(req.url, 'http://127.0.0.1')
      return url.searchParams.get('mobile') === '1'
    } catch {
      return false
    }
  }
  return false
}

export async function pipeYouTubeAudio(videoId, res, req) {
  if (!videoId) throw new Error('videoId required')

  const iosSafe = isIosSafeRequest(req)
  const upstream = await resolveUpstreamStream(videoId, iosSafe)
  if (upstream) {
    try {
      await pipeRemoteUrl(upstream, res, req)
      return
    } catch (error) {
      streamCache.delete(cacheKey(videoId, iosSafe))
      if (res.headersSent) return
      throw error
    }
  }

  if (iosSafe) {
    throw new Error('No iOS-compatible audio stream found for this video')
  }

  await pipeFromYtdl(videoId, res)
}

export async function handleYouTubeAudioRequest(videoId, res, req) {
  try {
    await pipeYouTubeAudio(videoId, res, req)
  } catch (error) {
    if (res.headersSent) {
      res.end()
      return
    }

    res.statusCode = 500
    res.setHeader('Content-Type', 'application/json')
    res.setHeader('Access-Control-Allow-Origin', '*')
    res.end(JSON.stringify({ error: error.message || 'Audio stream failed' }))
  }
}

export async function handleResolveRequest(videoId, res, req) {
  const iosSafe = isIosSafeRequest(req)
  const upstream = await resolveUpstreamStream(videoId, iosSafe)

  if (!upstream) {
    res.statusCode = 404
    res.setHeader('Content-Type', 'application/json')
    res.setHeader('Access-Control-Allow-Origin', '*')
    res.end(
      JSON.stringify({
        error: iosSafe
          ? 'No iOS-compatible audio stream found for this video'
          : 'No audio stream found for this video',
      }),
    )
    return
  }

  const proxyParams = new URLSearchParams({ videoId })
  if (iosSafe) proxyParams.set('mobile', '1')
  const proxyUrl = `/api/youtube-audio?${proxyParams}`
  const direct = isDirectPlayUrl(upstream.url)

  res.statusCode = 200
  res.setHeader('Content-Type', 'application/json')
  res.setHeader('Access-Control-Allow-Origin', '*')
  res.setHeader('Cache-Control', 'public, max-age=300')
  res.end(
    JSON.stringify({
      url: direct ? upstream.url : proxyUrl,
      contentType: upstream.contentType,
      direct,
      proxyUrl,
    }),
  )
}
